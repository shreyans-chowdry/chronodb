  ...
  return (
    <div className="flex min-h-screen flex-col bg-zinc-50/50">
      ...
      <header className="sticky top-0 z-30 border-b border-zinc-200 bg-white/90 backdrop-blur-xl">
        <div className="mx-auto flex h-14 max-w-[1600px] items-center justify-between px-4 sm:px-6">
          ...
          <div className="flex items-center gap-3">
            <Link
              href="/"
              className="flex items-center gap-2 rounded-lg px-2.5 py-1.5 text-zinc-600 transition-colors hover:bg-zinc-100 hover:text-zinc-900"
            >
      ...
      <main className="mx-auto w-full max-w-[1600px] flex-1 p-4 sm:p-6">
        {/* Commit selector */}
        <div className="mb-6 overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-xs animate-slide-up">
          ...
          <div className="grid grid-cols-1 gap-4 p-4 md:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                className="w-full rounded-lg border border-zinc-300 bg-white px-3 py-2 text-xs text-zinc-800 outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500/20"
              >
                {branches.map((b) => (
          ...
          {/* Actions */}
          <div className="flex items-center gap-3 border-t border-zinc-200 bg-zinc-50/30 px-4 py-3">
            {viewMode === "diff" ? (
              <button
                onClick={handleCompare}
                disabled={!commitA || !commitB || loading}
                className="flex items-center gap-2 rounded-lg bg-violet-600 px-4 py-2 text-xs font-semibold text-white shadow-md shadow-violet-500/20 transition-all hover:bg-violet-500 active:scale-95 disabled:cursor-not-allowed disabled:opacity-50"
              >
                {loading ? (
                  <>
        ...
        {mergeSuccess && mergeSuccess.status === "ok" && (
          <div className="mb-6 rounded-xl border border-emerald-200 bg-emerald-50 p-4 animate-fade-in shadow-xs">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-emerald-900">Merge Successful!</p>
                <p className="text-xs text-emerald-700">
                  Merged <span className="font-semibold">{branchB}</span> into{" "}
                  <span className="font-semibold">{branchA}</span>
                  {mergeSuccess.commit && (
                    <> · Commit <span className="font-mono font-medium">{mergeSuccess.commit.hash.slice(0, 8)}</span></>
                  )}
                </p>
              </div>
            </div>
        ...
        {!loading && !diffResult && !mergeResult && !mergeSuccess && !error && (
          <div className="flex flex-col items-center justify-center gap-4 rounded-xl border border-zinc-200 bg-white p-16 text-center shadow-xs animate-fade-in">
      ...
      {/* Footer */}
      <footer className="border-t border-zinc-200 bg-white py-3 text-center text-[10px] text-zinc-500">
