...
function CellValue({
  ...
  let classes = "px-3 py-2 text-sm font-mono transition-colors ";
  if (highlight && variant === "deleted") {
    classes += "bg-red-500/10 text-red-300 line-through decoration-red-400/60";
  } else if (highlight && variant === "added") {
    classes += "bg-emerald-500/10 text-emerald-300";
  } else if (highlight && variant === "modified") {
    classes += "bg-amber-500/8 text-amber-200";
  } else {
...
export default function DiffViewer({ diffData, commitA, commitB }: DiffViewerProps) {
  ...
  if (tableNames.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center gap-3 rounded-xl border border-zinc-700/40 bg-zinc-900/50 p-12 text-center backdrop-blur-sm">
    ...
    <div className="space-y-6 animate-fade-in">
      {tableNames.map((tableName) => {
        ...
        return (
          <div
            key={tableName}
            className="overflow-hidden rounded-xl border border-zinc-700/40 bg-zinc-900/50 backdrop-blur-sm"
          >
            {/* Table header */}
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-zinc-700/40 px-4 py-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-zinc-200">{tableName}</h3>
              </div>
            ...
            <div className="grid grid-cols-2 divide-x divide-zinc-700/40">
              {/* Left header: Before */}
              <div className="border-b border-zinc-700/40 bg-zinc-800/30 px-4 py-2">
                <div className="flex items-center gap-2">
                  <div className="h-2 w-2 rounded-full bg-red-400/60" />
                  <span className="text-[11px] font-medium text-zinc-400">Before</span>
                  <span className="font-mono text-[10px] text-zinc-600">{commitA.slice(0, 8)}</span>
                </div>
              </div>
              {/* Right header: After */}
              <div className="border-b border-zinc-700/40 bg-zinc-800/30 px-4 py-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-mono text-[10px] text-zinc-600">{commitB.slice(0, 8)}</span>
                </div>
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-zinc-700/40 bg-zinc-800/20">
                      <th className="px-3 py-2 text-left text-[10px] font-semibold uppercase tracking-widest text-zinc-500">
                        row_id
                      </th>
                      <th className="px-3 py-2 text-left text-[10px] font-semibold uppercase tracking-widest text-zinc-500">
                        status
                      ...
                      {fields.map((f) => (
                        <th key={f} className="px-3 py-2 text-left text-[10px] font-semibold uppercase tracking-widest text-zinc-500">
                          {f}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody className="divide-y divide-zinc-800/50">
                    {rows.map((row) => (
                      ...
                      >
                        <td className="px-3 py-2 font-mono text-xs text-violet-400/80">
                          {row.row_id}
                        </td>
